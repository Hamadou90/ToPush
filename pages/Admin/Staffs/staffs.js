...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      let allUsers = await fetch(
        `${process.env.NEXT_PUBLIC_HOST}/api/AdminAccess/Users/getAllUsers`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        }
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    let a = await fetch(
      `${process.env.NEXT_PUBLIC_HOST}/api/AdminAccess/Users/updateUser`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        <ToastContainer
          ...
          pauseOnHover
        />
        <h2 className="fs-1 fw-bold mt-5 text-center">Liste des Employés</h2>
        <div className="row mt-3">
          ...
          <div className="col-md-8 mt-1">
            <Link href={`${router.pathname.replace('/staffs', '')}/addStaff`}>
              <button
                type="button"
              ...
              >
                <i className="bi bi-plus-square-dotted"></i>&nbsp;Créer un nouvel employé
              </button>
            </Link>
                      ...
                      <tr key={index}>
                        <td scope="row">
                          <Link
                            ...
                            passHref={true}
                            href={`/user/${users[item].user_id}`}
                          >
                            <span>#000{users[item].user_id}</span>
                          </Link>
                        </td>
                        <td>{users[item].username}</td>
                        <td>{users[item].email}</td>
                        <td>
                          {String(users[item].created_on).substr(0, 10) +
                            " à " +
                            String(users[item].created_on).substr(11, 8)}
                        </td>
                        <td>
                          {" "}
                          <span
                    ...
                    <div className="col-md-12">
                      <table className="table table-responsive table-bordered table-striped">
                        <tbody>
                          <tr>
                            <td>Identifiant:</td>
                            <td>#000{selectedUser && selectedUser.user_id}</td>
                          </tr>
                          <tr>
                            <td>Nom et Prénom:</td>
                          ...
                          <tr>
                            <td>Direction:</td>
                            <td>{selectedUser && selectedUser.direction_acronym}</td>
                          </tr>
                          <tr>
                            <td>Type de Compte:</td>
                            <td>{selectedUser && selectedUser.category === 'Simple User'? 'Employé': 'Directeur' }</td>
                          </tr>
                          <tr>
                            <td>Date de Création:</td>
                            <td>{selectedUser.created_on && selectedUser.created_on.substr(0, 10)}</td>
                          </tr>
                          <tr>
                            <td>Compte Activé?:</td>
              ...
              <div className="modal-dialog">
                <div className="modal-content">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    required
                    value={selectedUser.direction_id}
                  >
                    <option value={null}>
