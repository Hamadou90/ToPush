...
// From SearchQuery component Start

// From SearchQuery component End
...
// Section Fetch Other group members
  ...
  const handleSearchSubmit = async (e) => {
    ...
    const fetchTaskData = async () =>{
      const theSpecificTask = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/Tasks/getAllTasks?q=${query}`,
      {
          method: "GET",
          headers: { "Content-Type": "application/json" }
        })
  ...
  return (
    <div className={styles.container}>
      <Head>
        ...
        <meta name="description" content="Système de Gestion de Suivi des Tâches" />
      ...
      <main className={styles.main}>
        <ToastContainer
            ...
            pauseOnHover
          />
        <h2 className="fs-1 fw-bold mt-5">Liste des Tâches</h2>
              ...
              {records &&
                Object.keys(records).map((item, index) => {
                  return (
                    <tr key={index}>
                      <td scope="row">
                        <Link className="cursor-pointer"  passHref={true} href={`/task/${records[item].taskslug}`}>
                         <span>#000{records[item].task_id}</span>
                        </Link>
                        
                      </td>
                      <td>{records[item].tasktitle}</td>
                      <td>{records[item].starting_date.substr(0, 10)}</td>
                      <td>{records[item].ending_date.substr(0, 10)}</td>
                      <td>
                          {" "}
                        <span
                      ...
                      <button
                          ...
                          onClick={() => {setSelectedTask(records[item]); setSelectedTask_id(records[item].task_id); setTeamleader_id(records[item].teamleader_id); }}
                      ...
                      <button
                          onClick={() =>{
                            (confirm("êtes vous sûre de vouloir supprimer?")) ? triggerDeletion(records[item].task_id) : '';}
                          }
                          type="button"
                      ...
                      {records[item].status && records[item].status === "Under Process" &&
                        <button
                            type="button"
                            className="btn btn-sm btn-primary"
                            onClick={ () =>{ (confirm("Vous êtes sur le point de spécifier cette tâche comme terminée")) ? triggerStatusChangeHandler(records[item].task_id) : '';} }
        ...
        <nav className='col-5 offset-3'>
                  <ul className="pagination">
                      {currentPage !== 1 && <li className="page-item mx-1">
                      ...
                      <div className='w-[50vw] d-flex overflow-y-hidden overflow-x-auto'>
                          ...
                          {currentPage !== npage && <li className="page-item mx-1">
