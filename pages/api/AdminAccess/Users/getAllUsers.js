  ...
  try {
    if (connect) {
      ...
      if (req.method == 'GET') {
        ...
        let theUsers = await connect.query("SELECT * FROM users u, directions d WHERE d.direction_id = u.direction_id ORDER BY activated ASC, deleted ASC");
        // let theUsers = await connect.query("SELECT * FROM bookings AS b, \"Users\" AS u, packages AS p WHERE p.package_id = b.package_id AND b.user_id = u.\"User_id\" AND u.\"User_id\" = $1", [
        //   req.body.user_id
        // ]);
